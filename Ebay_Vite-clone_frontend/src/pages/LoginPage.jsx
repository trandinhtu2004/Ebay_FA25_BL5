...
// URL Backend (Nên dùng biến môi trường, fallback về localhost)
...
const LoginPage = ({ initialMode = 'login' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Gọi API bằng axios (nhất quán với AuthContext)
      ...
      if (isRegister) {
        ...
        // --- QUAN TRỌNG: Dùng hàm login của Context để cập nhật Global State ---
        // data.token: JWT token
        // data.user: User info object
    ...
    <div className="min-h-screen flex font-sans text-[#191919] bg-white overflow-hidden">
      ...
      <header className="px-6 py-4 flex justify-between items-center bg-white z-20 absolute w-full top-0 left-0">
        ...
        <div className="text-sm z-20 hidden sm:block">
          {isRegister ? (
            <>Already have an account? <span onClick={() => toggleMode('login')} className="text-[#0654ba] underline hover:no-underline font-medium cursor-pointer">Sign in</span></>
          ) : (
            <>New to eBay? <span onClick={() => toggleMode('register')} className="text-[#0654ba] underline hover:no-underline font-medium cursor-pointer">Create account</span></>
          )}
        </div>
      </header>
      ...
      <div className="w-full lg:w-[45%] flex flex-col justify-center items-center px-6 py-12 lg:py-0 overflow-y-auto h-full">
        <div className="w-full max-w-[440px] pt-16 lg:pt-0">
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            {isRegister && (
               <FloatingInput label="Username" name="username" value={formData.username} onChange={handleChange} required />
            )}

            <FloatingInput label="Email" type="email" name="email" value={formData.email} onChange={handleChange} required />
            
            <div className="relative">
              <FloatingInput label="Password" type={showPassword ? "text" : "password"} name="password" value={formData.password} onChange={handleChange} required />
            ...
            <button disabled={loading} className={`w-full bg-[#3665f3] hover:bg-[#2b50c4] text-white font-bold h-12 rounded-full mt-2 transition-colors text-[16px] ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}>
