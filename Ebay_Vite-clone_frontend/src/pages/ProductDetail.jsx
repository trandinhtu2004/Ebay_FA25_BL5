...
import { BsHeart, BsStarFill, BsShieldCheck, BsTruck, BsPerson, BsShop, BsEnvelope } from 'react-icons/bs';
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    fetchData();
    // Reset scroll lên đầu trang khi id thay đổi
    window.scrollTo(0, 0);
  }, [id]);
  ...
  return (
    <div className="bg-white min-h-screen font-sans text-[#191919]">
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-2 text-xs text-gray-500">
         <Link to="/" className="hover:underline text-blue-700">eBay</Link> {'>'} 
         <Link to={`/all-products?category=${encodeURIComponent(product.category?.name)}`} className="hover:underline text-blue-700">
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-6">
        <div className="flex flex-col lg:flex-row gap-8">
                 ...
                 <div className="flex gap-2 overflow-x-auto pb-2">
                    {product.images.map((img, idx) => (
                        <div 
                            ...
                            className={`w-16 h-16 border rounded-md cursor-pointer flex-shrink-0 hover:opacity-80 ${activeImage === img ? 'border-blue-500 border-2' : 'border-gray-300'}`}
                        >
          ...
          <div className="w-full lg:w-[40%]">
            ...
            <div className="flex items-center gap-1 text-sm mb-4 border-b border-gray-200 pb-4">
                 <div className="flex text-yellow-500">
                    {Array.from({length: 5}, (_, i) => (
                        <BsStarFill key={i} className={i < (product.rating || 0) ? '' : 'text-gray-300'} />
                    ))}
                 </div>
            ...
            <div className="bg-gray-50 p-4 rounded border border-gray-200 mb-6">
                 <div className="flex items-baseline gap-2 mb-2">
                    ...
                    <span className="text-3xl font-bold text-black">US ${product.price.toLocaleString()}</span>
                 </div>
                 ...
                 <div className="flex items-center gap-4 mb-6">
                    ...
                    <span className="text-xs text-gray-500">
                        {product.stock > 10 ? 'More than 10 available' : `${product.stock} available`}
                    </span>
                 </div>

                 <div className="flex flex-col gap-3">
                    <button onClick={() => toast.info('Chức năng "Buy It Now" chưa phát triển')} className="w-full bg-[#3665f3] hover:bg-[#2b50c4] text-white font-bold py-3 rounded-full transition">Buy It Now</button>
                    <button onClick={handleAddToCart} className="w-full bg-[#d6e3ff] text-[#0654ba] hover:bg-[#c3d5f5] font-bold py-3 rounded-full transition">Add to cart</button>
             ...
             <div className="space-y-3 text-sm text-gray-700">
                <div className="flex gap-3">
                    ...
                    <div><span className="font-bold">Shipping:</span> <span className="text-xs text-gray-500">Calculate at checkout</span></div>
                </div>
                <div className="flex gap-3">
                    <BsShieldCheck className="text-xl" />
                    <div><span className="font-bold">eBay Money Back Guarantee.</span></div>
                </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-[20%]">
             <div className="border border-gray-200 rounded p-4 shadow-sm bg-white">
                <h3 className="font-bold text-sm mb-3 text-gray-900">About this seller</h3>
                ...
                <div className="flex items-start gap-3 mb-4">
                    ...
                    <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-100 border border-gray-300 flex-shrink-0">
                        <img 
                            src={seller.avatarURL || "https://ir.ebaystatic.com/cr/v/c01/buyer_dweb_individual.jpg"} 
                            alt={seller.username} 
                            className="w-full h-full object-cover"
                            onError={(e) => {e.target.src = "https://ir.ebaystatic.com/cr/v/c01/buyer_dweb_individual.jpg"}} // Fallback nếu link ảnh lỗi
                        />
                    </div>
                    
                    <div className="overflow-hidden">
                        <Link to="#" className="text-blue-700 font-bold hover:underline block truncate text-base" title={seller.username}>
        ...
        <div className="mt-12 border-t border-gray-200 pt-8">
            <h2 className="text-xl font-bold mb-4 bg-gray-100 p-2 inline-block rounded-t border-t border-l border-r border-gray-200">Description</h2>
            ...
            <div className="mt-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    {similarProducts.map((prod) => (
                        <Link to={`/product/${prod._id}`} key={prod._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-lg transition cursor-pointer bg-white block no-underline">
                            <div className="relative w-full aspect-square bg-gray-100 rounded-lg overflow-hidden mb-3 border border-gray-200">
                                <img
                                    src={prod.images && prod.images.length > 0 ? prod.images[0] : 'https://placehold.co/200x200?text=No+Image'}
                                    alt={prod.title}
                                    className="w-full h-full object-cover"
                                />
                            </div>
                            ...
                            <div className="font-bold text-lg text-gray-900">${prod.price.toLocaleString()}</div>
                        </Link>
                    ))}
                </div>
            </div>
